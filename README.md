...
A full-stack IT helpdesk and ticketing system. Supports three distinct roles (Admin, Technician, End User), automatic SLA tracking, email notifications, a self-service portal, and a knowledge base.
...
- **Email notifications** — Transactional emails via Microsoft 365 SMTP for ticket assignment, reassignment, new replies, and resolution. Queued at ≤ 30/min to respect M365 rate limits; best-effort (failures are logged and never affect API responses)
...
- **Remember Me** — Optional persistent login session (30-day cookie)
- **First-login password change** — Users are forced to set a new password on their first login
...
2. Create a `.env` file in the project root:
   ...
   # Microsoft 365 SMTP — leave blank to disable email notifications
   SMTP_HOST=smtp.office365.com
   SMTP_PORT=587
   SMTP_USER=no-reply@your-domain.com
   SMTP_PASSWORD=your_smtp_password
   SMTP_FROM_NAME=helpIT Notifications
...
1. Create `backend/.env`:
   ...
   # Optional — omit to run without email notifications in dev
   SMTP_HOST=smtp.office365.com
   SMTP_PORT=587
   SMTP_USER=no-reply@your-domain.com
   SMTP_PASSWORD=your_smtp_password
...
| `CORS_ORIGIN`        | Yes      | Allowed frontend origin — also used as the base URL in email links |
| `PORT`               | No       | API port (default: `3001`)                                        |
| `NODE_ENV`           | No       | Set to `production` to enable secure cookies and harden responses |
| `SMTP_HOST`          | No       | SMTP server hostname (e.g. `smtp.office365.com`)                  |
| `SMTP_PORT`          | No       | SMTP port — `587` for STARTTLS                                    |
| `SMTP_USER`          | No       | SMTP login username                                               |
| `SMTP_PASSWORD`      | No       | SMTP login password                                               |
| `SMTP_FROM_NAME`     | No       | Display name in From header (default: `helpIT Notifications`)     |
| `SMTP_FROM_ADDRESS`  | No       | From email address                                                |

> All `SMTP_*` variables are optional. If any are missing the app starts normally and email notifications are silently disabled.

### Docker Compose

Docker Compose reads these from the root `.env`:
...
When SMTP is configured, emails are sent automatically on these events:

| Event | Recipients |
|-------|-----------|
| Ticket created (auto-assigned) | Assigned technician |
| Ticket reassigned | New assignee only (skipped if same person) |
| Public comment added | Requester + assigned technician, excluding the comment author |
| Ticket resolved or closed | Requester only |

All email subjects are prefixed `[helpIT #<id>]` for email threading. Each recipient receives a separate email. Internal (staff-only) comments do not trigger notifications to the requester.
...
│   │   ├── services/
│   │   │   └── emailService.js # Nodemailer wrapper + notification functions
